...
def add_nova_to_excel():
    # 准备新星组数据
    nova_data = []
    
    # 冬季赛 - 女性组
    nomination_file = os.path.join(root_dir, 'data', 'nomination', 'nova', 'winter', 'female', '03-nova-winter-female-nomination.json')
    ...
    nova_data.extend(winter_female)
    
    # 冬季赛 - 男性组
    nomination_file = os.path.join(root_dir, 'data', 'nomination', 'nova', 'winter', 'male', '04-nova-winter-male-nomination.json')
    with open(nomination_file, 'r', encoding='utf-8') as f:
        nomination_data = json.load(f)
    ...
    eliminate_count = 1
    for i, char in enumerate(nomination_data['data'], 1):
        status = '已晋级至新星组春季赛预选赛' if char['is_advanced'] else '未晋级'
        char_id = f"NFS{advance_count:03d}" if char['is_advanced'] else f"ENFS{eliminate_count:03d}"
        if char['is_advanced']:
            advance_count += 1
        else:
            eliminate_count += 1
    ...
    # 保存为Excel
    output_file = os.path.join(root_dir, 'data', 'characters', 'ISML2024-characters-nova.xlsx')
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        df.to_excel(writer, sheet_name='Nova', index=False)
    
    print(f"冬季赛和春季赛数据已添加到: {output_file}")
