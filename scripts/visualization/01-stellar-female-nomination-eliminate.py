...
df_eliminate = df_voted.tail(39) 

total_count = len(df_eliminate)
votes = df_eliminate['得票数'].tolist()[::-1]  
labels = [f"{role}（{anime}）" for role, anime in zip(df_eliminate['角色'], df_eliminate['IP'])][::-1]  
...
def create_vote_chart():
    ...
    output_file = os.path.join(output_dir, '01-stellar-female-nomination-eliminate.html')
