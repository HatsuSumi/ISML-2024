...
csv_path = os.path.join(root_dir, 'data', 'nomination', 'stellar', 'female', '01-female-nomination.csv')
df = pd.read_csv(csv_path, encoding='utf-8')

df['得票数'] = pd.to_numeric(df['得票数'].replace('-', '0'))

df_voted = df[df['得票数'] > 0].sort_values('得票数', ascending=True)
...
advance_data = [v if i >= len(votes) - 92 else None for i, v in enumerate(votes)]
eliminate_data = [v if i < len(votes) - 92 else None for i, v in enumerate(votes)]
...
<include src="../../../templates/header.html"></include>
        ...
        <div class="content-wrapper">
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {{
                    ...
                    title.textContent = '12.31-01.07 - 恒星组提名-女性组别';
    ...
    <script src="../../../js/chart-legend/01-stellar-nomination-chart-legend.js"></script>
...
def create_vote_chart():
    try:
        ...
        output_file = os.path.join(output_dir, '01-stellar-female-nomination.html')
